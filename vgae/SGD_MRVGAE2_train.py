...
def get_scores(posA, negA, pos_graph,neg_graph):
    ...
    def sigmoid(x):
        return 1 / (1 + np.exp(-x))
    preds = posA.cpu().detach().numpy()
    pos = np.zeros_like(preds,dtype=np.int16)
    pos[:,1] = 1
    preds_neg = negA.cpu().detach().numpy()
...
def train(model,predictor,device,args,train_g):
    ...
    for epoch in range(args.num_epoch):
        ...
        model.eval()
        posA,posX,negA,negX = model.inference(train_g.to(torch.device(device)),val_pos_graph.to(torch.device(device)),val_neg_graph.to(torch.device(device)),node_features.to(device),temp=0.5)
