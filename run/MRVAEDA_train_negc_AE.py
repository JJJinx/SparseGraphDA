...
class MRVAEDA(torch.nn.Module):
    def __init__(self, in_dim,hidden_dim,categorical_dim,device, **kwargs):
        ...
        self.A_classifier = relation_classifier(hidden_dim[1],hidden_dim[1]//2,categorical_dim.item()) #classify the node pair's class
...
def train(models,src_dataloader,tgt_dataloader,
    ...
    for batch_idx,data in enumerate(zip(src_dataloader,tgt_dataloader)):
        ...
        src_X_recon,src_A_pred,src_domain_pred = models(source_node_feat, source_edge_index,src_batch_np,'source',rate)
        tgt_X_recon,tgt_A_pred,tgt_domain_pred = models(target_node_feat, target_edge_index,tgt_batch_np,'target',rate)
        ...
        # backward
        loss = loss_cls+loss_recon+loss_da
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def validate(models,dataloader,all_node_pair_label,mapping_matrix,node_feat,node_label,edge_index,domain,device,rate):
    ...
    for batch_np,_ in dataloader:
        _,np_pred_temp,_ = models(node_feat.to(device),edge_index.to(device),batch_np.to(device),domain,rate)
