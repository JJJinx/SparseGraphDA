...
def train(models,source_node_feat,target_node_feat,source_edge_index,target_edge_index,
    ...
    '''
    src_np_pos_label_idx = torch.stack(torch.where(src_all_node_pair_label<21)).T # pos label<21
    tgt_np_pos_label_idx = torch.stack(torch.where(tgt_all_node_pair_label<21)).T
    ...
    src_np_neg_label_idx = torch.stack(torch.where(src_all_node_pair_label>=21)).T
    tgt_np_neg_label_idx = torch.stack(torch.where(tgt_all_node_pair_label>=21)).T
    ...
    src_np_label = src_all_node_pair_label[src_node_pair[:,0],src_node_pair[:,1]]  # src_np_label shape [NP,] src_all_node_pair_label shape [N,N]
    ...
    loss_cls = F.cross_entropy(src_A_pred,src_np_label.type(torch.long).to(device))
...
if __name__ == "__main__":
    ...
    categorical_dim = (max(max_pos_np_label,max_neg_np_label)-0+1).item()
    mapping_matrix = generate_mapping_M_minus_class(node_label_num,categorical_dim,max_pos_np_label+1)
    # print(max_pos_np_label) 20
    # raise RuntimeError
